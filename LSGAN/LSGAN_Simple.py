...
ds = tv.datasets.ImageFolder('D:/HS_Kang/GitHub/PyTorch_DCGAN/img_align_celeba', trans)
dataloader = tc.utils.data.DataLoader(dataset=ds, batch_size=batch_sz, shuffle=True)
...
for ep in range(epoch_sz):
    ...
    z = tc.randn(16, latent_sz).view(-1, latent_sz, 1, 1).to(device)
    fake_images = G(z)
    out = fake_images
    out = image_range(out)
    save_image(out, os.path.join(result_path, 'fake_img {}.png'.format(ep)))

    tc.save(G.state_dict(), 'G_t.ckpt')
    tc.save(D.state_dict(), 'D_t.ckpt')
